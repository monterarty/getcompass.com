...
var Detector = new oldBrowserDetector({ c: 87, f: 73, o: 70, s: {d:12} }, function() {
	showOldBrowserMsg()
})
...
function checkValidationFormOnSubmit(formClassName) {
	...
	const onSubmitHandler = (event) => {
		if (formValidation(form)) {
			...
			form.querySelector('.w-button').value = "Submit";
			return false;
		}
	};
...
$(document).on('closing', '.remodal', function (e) {
	...
	setTimeout(function () {
		...
		if (formInModal.length) {
			...
			formInModal.find('.button-submit').val('Submit');
			formInModal[0].reset();
			formInModal.show();
...
clipboard.on('error', function (e) {
	showCopyNote('Failed to copy the link. Your browser version is probably out of date.', true);
});
...
function showCopyNote(text, error) {
	...
	if (error) {
		...
		$('.event__message').html('Download link for PC is copied');
	}
...
function tabsSwipeHandler(linkEl, direction) {
	var currentMenu = linkEl.closest('.w-tabs').find('.w-tab-menu'),
	...
	if (direction == 'left' && $(window).width() < 768) {
		...
		if (prevLink.length < 1) {
			...
			prevLink.click();
			allSliders[$('.w-tab-menu').index(currentMenu)].update()
		}
	}
}
...
$('.target__tab-menu-block .w-tab-link').on('click', function () {
	...
	if ($(window).width() > 767) {
		...
		if (tabIndex != allSliders[tabsElIndex].snapIndex)
...
$('form').on('submit', function (e) {
	...
	if (formValidation(form[0])) {
		formData.set('team_size', typeof formData.get('team_size') == 'string' ? +formData.get('team_size') : formData.get('team_size'));
		...
		$.ajax({
			...
			statusCode: {
				423: function (result) {
					errorMessage.find('div').html('You have reached the limit of the requests. Please try again in 2&nbsp;hours.');
				}
			},
			...
			error: function (xhr, resp, text) {
				errorMessage.find('div').html('An error occurred while submitting the form, please check your connection or try again later.');
				button.val(initialBtnText);
				errorMessage.show();
				setTimeout(function () {
					errorMessage.hide();
